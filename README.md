...
| 🌍 CO₂ Conversion | kg CO₂ → 🌳 trees equivalent → 🚗 km avoided |
| 🏆 GreenPulse Score | Weighted composite sustainability index (0–100) |
| 📊 Leaderboard | Block ranking with gamified incentives driven by dynamic API |
| 🧠 AI Engine | Statistical Standard Deviation anomaly detection + LinearRegression forecasts |
...
# Triggers alert if a day's usage exceeds the mean by 1.5x standard deviations
alert = "High" if (val - mean) > 1.5 * std else "Normal"
```
...
├── frontend/
│   ├── src/
│   │   ├── components/       # UI building blocks (StatCards, ChartsGrid)
│   │   ├── hooks/            # Custom API fetching (useBlockData, useLeaderboard)
│   │   ├── pages/            # Main Dashboard view
│   │   ├── utils/            # Chart configuration helpers
...
- Per-room optimization  
- Real-time dashboard streaming  
- GenAI integration for dynamic LLM-driven sustainability tips  
- Mobile companion app  
